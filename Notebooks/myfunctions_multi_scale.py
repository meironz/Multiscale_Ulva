...
def multi_N_f_un(
    ...
    for i_reactor in range(n_reactors):  # loop that constructs the coupled ODEs
        ...
        if Nint >= Nintcrit:
            ...
            fN = ((Nint - Nintmin) / Nint) / (
                (Nintcrit - Nintmin) / Nintcrit
            )  # units: [ ]
        fP = 1  # (N:P < 12)

        # density - light penetration effects:
        SD = m * 1.785 / 2 * 1000  # Stocking Density. units: [g DW/ m^2]
        I_average = (I0 / (K0 * Z + Ka * SD)) * (
            1 - np.exp(-(K0 * Z + Ka * SD))
        )  # units: [umul photons/(m^2 s)]
        ...
        fT = np.exp(
            -2.3 * ((Temp - Topt) / (Tx - Topt)) ** n
        )  # Temp from temperature data
        ...
        if i_reactor > 0:
            Nsea_upstream = y[
                i - 4
            ]  # y is a vector state, 4 state variables backwards is previous Nsea

        # Reactor Nsea -> Next -> Nint -> m and feedback
        dNsea = (
            Qsea * (Nsea_upstream * (1 - dilution) - Nsea) - Qp * (Nsea - Next)
        ) / (1.785 * 1000)
        dNext = (
            Qp / (1.785 * 1000) * (Nsea - Next) - Neff * uN * m
        )  # [umol N/l/h]
        dNint = (
            umol_to_percent_DW * Neff * uN - Nint * miu * g
        )  # units: [%g N/g DW/h]
...
def plot_result_un(
    T, NSEA, NEXT, NINT, M, resolution, n_reactors, Nsea, fig, ax, season
):
    """
    ...
    sns.set_palette(
        sns.cubehelix_palette(
            20, start=2, rot=0, dark=0.2, light=0.7, reverse=True
        )
    )
    ...
    ax[2].set_xlabel('Days', color='black', fontsize=12,weight="bold"
    )
    name = 'fig6.jpg'
...
def plot_result_Nsea(T, NSEA, NEXT, NINT, M, Nsea, i_reactor):
    ...
    for i in reactors:
        ax[0].plot(
            t, NSEA[i], ".", color=colors[reactors.index(i)], markersize=2
        )
        ax[0].set_ylim([0, Nsea * 1.2])
        # ax[1].plot(t,NEXT[i],'.',color='black', markersize=2) # removed Next plot as it is redundant for the figure
        # ax[1].set_ylim([0,Nsea*1.2])
        ax[1].plot(
            t, NINT[i], ".", color=colors[reactors.index(i)], markersize=2
        )
        ax[1].set_ylim([0.5, 4.5])
    ...
    ax[2].set_xticklabels(
        [str(i) for i in xlabels], rotation=45, fontsize=10, weight="bold"
    )
...
def plot_result_seasons(
    T, NSEA_seasons, NINT_seasons, M_seasons, Nsea, NSEA_seasons_f
):
    """
    1. Always write documentation
    2. Specify what the function does
    3. Specify what are the inputs and what are the outputs
    4. Don't send an unnecessary inputs if you return them, create them

    Plots ...

    Inputs:
        T : numpy array of integers or list of arrays or what?

    Outputs: 
        fig, ax : a pointer to the figure and axes objects for later reuse
    """
    ...
    for i in seasons:
        ax.flat[2].plot(
            t,
            NSEA_seasons[seasons.index(i)],
            "-",
            color=colors[seasons.index(i)],
            markersize=2,
        )
        ax.flat[2].set_ylim([0, Nsea * 1.2])
        ax.flat[1].plot(
            t,
            NINT_seasons[seasons.index(i)],
            "-",
            color=colors[seasons.index(i)],
            markersize=2,
        )
    ...
    ax.flat[2].set_xticklabels(
        [str(i) for i in xlabels], rotation=45, fontsize=12
    )
    ax.flat[3].set_xticklabels(
        [str(i) for i in xlabels], rotation=45, fontsize=12
    )
...
def plot_result_Qp_2cages(
    ...
    for i in Qps:
        ax.flat[0].plot(
            t,
            NSEA_Qp1[Qps.index(i)],
            "-",
            markersize=2.5,
            color=colors[Qps.index(i)],
            linewidth=2,
        )
        ax.flat[0].plot(
            t,
            NSEA_Qp2[Qps.index(i)],
            ":",
            markersize=3,
            color=colors[Qps.index(i)],
            linewidth=2.5,
    ...
    ax.flat[3].set_xticks(np.linspace(T[0][0], T[0][0] + 14 * 24, 15))
    ax.flat[2].set_xticklabels(
        [str(i) for i in xlabels], rotation=45, fontsize=12, weight="bold"
    )
    ax.flat[3].set_xticklabels(
        [str(i) for i in xlabels], rotation=45, fontsize=12, weight="bold"
    )
    ...
    legend = ax.flat[3].legend(
        [
            "Qp = 460 l/h, first reactor",
            "Qp = 460 l/h, last reactor",
            "Qp = 15 l/h, first reactor",
            "Qp = 15 l/h, last reactor",
            "Qp = 0 l/h, all reactors",
        ],
        ncol=3,
        fontsize="x-large",
        framealpha=0.8,
